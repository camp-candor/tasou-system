...
import * as ActCtr from "../../00.mythos.unit/mythos.action";
...
export const updateMythos = async (cpy: MythosModel, bal: MythosBit, ste: State) => {

  

  if (cpy.opened == 0) {
    bit = await ste.hunt(ActCtr.OPEN_MYTHOS, {})
  ...
  if (bal.val === 1) {
    bit = await ste.hunt(ActCtr.ADVANCE_MYTHOS, {})
    bit = await ste.hunt(ActCtr.OUTPUT_MYTHOS, {})
    dat = bit.ctlBit.dat
    bal.slv({ ctlBit: { idx: "update-mythos", dat } });
    return cpy;
  }
