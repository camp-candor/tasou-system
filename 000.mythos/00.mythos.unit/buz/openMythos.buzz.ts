...
import * as ActCtl from '../mythos.action'
...
export const openMythos = async (cpy: MythosModel, bal: MythosBit, ste: State) => {
  ...
  if (typeof document === 'undefined') return bal.slv({ ctlBit: { idx: "open-mythos", src: 'no document' } });
  if (document === undefined) return bal.slv({ ctlBit: { idx: "open-mythos", src: 'no document' } });

  await new Promise(resolve => setTimeout(resolve, 10));

  await ste.hunt(ActCtl.DATA_MYTHOS, { src: 'color-list' })
 ...
 // var container = dat['bit']
  ...
  bal.slv({ ctlBit: { idx: "open-mythos" } });
  return cpy;
};
